...
;; Load rooms without random mobs/puzzles/traders for deterministic tests
(binding [rooms/*enable-random-content* false]
...
(def-command-test test-game-commands
  ;; test attack/use/equip/stats via the execute path
  (binding [*err* (java.io.StringWriter.)]
    ;; attack (spawned mob) should return a hit or kill message
    (is (re-find #"You hit|You killed|No such mob" (execute "attack")))
    ;; equip a sword
    (is (re-find #"equipped" (execute "equip sword")))
    ;; add a potion to inventory and use it
    (dosync (alter player/*inventory* conj :hp-small))
    (is (re-find #"You used" (execute "use hp-small")))
    ;; stats shows a Stats: string
    (is (re-find #"Stats:" (execute "stats")))))
