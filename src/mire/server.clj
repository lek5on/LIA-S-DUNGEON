...
(defn- cleanup []
  "Drop all inventory, remove player from room, lobby, and player list."
...
(defn- mire-handle-client [in out]
  (binding [*in* (io/reader in)
    ...
    (binding [player/*name* (get-unique-player-name (read-line))
      ...
      (try
        ;; Track the player's stream immediately so lobby broadcasts work.
        (dosync
         (commute player/streams assoc player/*name* *out*))

        ;; Lobby: wait for everyone to ready up before entering the world.
        (let [start-signal (lobby/register-player! player/*name*)
          ...
          (loop []
            (cond
              ...
              (println "\nAll players ready. Launching the dungeon...")
                ...
                (cond
                  ...
                  (let [line (str/trim (:line res))
                    ...
                    (case cmd
                      ...
                      (println "Unknown lobby command. Try: ready, unready, status, quit."))
                    (when-not (#{"quit"} cmd)
                      (print lobby/prompt) (flush)
                      (recur)))))))

          ;; Enter the world after lobby start.
