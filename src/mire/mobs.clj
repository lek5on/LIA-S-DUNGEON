...
(def mobs
  {:zombie {:name "Зомби" :hp 30 :max-hp 30 :damage 5 :xp 20 :gold 10}
   :skeleton {:name "Скелет" :hp 25 :max-hp 25 :damage 6 :xp 25 :gold 15}
   :witch {:name "Ведьма" :hp 40 :max-hp 40 :damage 8 :xp 40 :gold 30}
   :berezutskie {:name "Братья Березуцкие" :hp 70 :max-hp 70 :damage 14 :xp 80 :gold 60}
   :pudge {:name "Пудж" :hp 100 :max-hp 100 :damage 10 :xp 90 :gold 70}
   :ghost {:name "Призрак" :hp 20 :max-hp 20 :damage 12 :xp 35 :gold 25}
   :slenderman {:name "Слендермен" :hp 80 :max-hp 80 :damage 15 :xp 120 :gold 100}
   :herobrine {:name "Херобрин" :hp 120 :max-hp 120 :damage 18 :xp 150 :gold 120}
   :varpach {:name "Варпач" :hp 60 :max-hp 60 :damage 9 :xp 50 :gold 40}})
...
(defn mob-attack!
  ...
  (let [dmg (:damage @mob-atom 0)]
    ...
    (str (:name @mob-atom) " бьёт вас на " dmg " урона.")))
...
(defn player-attack-mob!
  ...
  (let [pdmg (:damage @player-stats)]
    ...
    (str "Вы нанесли " (:name @mob-atom) " " pdmg " урона.")))
