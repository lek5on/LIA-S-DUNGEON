...
(def room-descriptions
  ["Вы стоите в тускло освещённой комнате с каменными стенами."
   "Перед вами открывается огромная подземная пещера."
   "Вы находитесь в узком туннеле, вырубленном в скале."
   "Эта комната пахнет сыростью и гнилью."
   "Древние письмена покрывают стены."
   "Величественный зал с высоким потолком простирается перед вами."
   "Пол здесь скользкий от влаги."
   "Груды камней говорят о недавнем обвале."
   "Факелы мерцают на стенах, отбрасывая танцующие тени."
   "Затхлый запах древних сокровищ наполняет воздух."
   "Кристаллические образования мерцают в темноте."
   "Стены здесь несут следы битвы."
   "Загадочные символы выгравированы на камне."
   "Вода равномерно капает с потолка."
   "Воздух здесь холодный и гнетущий."])
...
;; NPC/Trader types
(def trader-types
  [{:name "Старый торговец" :desc "Опытный торговец с множеством товаров"}
   {:name "Отшельник" :desc "Загадочный отшельник из глубин"}
   {:name "Кузнец" :desc "Грубый кузнец с мощным снаряжением"}
   {:name "Алхимик" :desc "Алхимик, варящий странные зелья"}
   {:name "Гадалка" :desc "Мистическая гадалка с загадочными словами"}])
...
(defn generate-room-content []
  (let [room-type (rand)
    ...
    (cond
      ...
      (< room-type 0.90)
      (assoc base :puzzle (puzzles/random-puzzle))
  ...
  (let [rooms {}]
    ...
    (reduce (fn [rooms [x y]]
              (let [room-id (keyword (str "room-" x "-" y))
                ...
                (when (< y 2) (dosync (alter exits assoc :south (keyword (str "room-" x "-" (inc y))))))
